# Chat-Server

This repository is dedicated to learning how basic network programming is done and how to establish connections between multiple clients through a server.

### **The goals of this project are the following:**
1. [x] Send message from client to server and respond with acknowledgement.
2. [x] Keep session open between client and server until a specific input occurs.
3. [x] Send messages from various clients.
4. [ ] Redirect traffic from each client to all other clients.
5. [ ] Send confirmation of received messages by clients to original client.
6. [ ] Give each client a unique username and password defined when first accesing the program. Store credentials locally and on server bank (username + hash of password).
7. [ ] Allow clients to login instead of registering.
8. [ ] Make conversations work based on petitions from client to client.
9. [ ] Store conversations in local files. Server stores messages until they have been received and acknowledged by receiving party.
10. [ ] GUI of chat conversations.

### **How to use this project:**
This will be defined later on.

### **Goals if I continue the project:**
1. Group creation.
2. Send files between clients.
3. User profile icons.
4. Some sort of session implementation to avoid logging in each time the client program runs.
5. Encryption of messages.